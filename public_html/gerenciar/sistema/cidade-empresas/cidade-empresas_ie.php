...
if($_POST){
	if($ac=='i' && in_array($Mod_ID."a", $_SESSION['_user']['Permissao_Modulos'])){
		$CidadeEmpresas->Grava($_POST);
	}elseif($ac=='e' && in_array($Mod_ID."e", $_SESSION['_user']['Permissao_Modulos'])){
		$CidadeEmpresas->Edita($_POST, $id);
	}

	header("Location:".urlBase."$Mod[0]/$Mod[1]/cidade-empresas.php");
}else{
	//TOKEN
...
$smarty->assign('ac', $ac);
$smarty->assign('dados', $CidadeEmpresas->Dados($id));
$smarty->assign('cidades', $Cidades->listaCidades());
$smarty->display('sistema/cidade-empresas/cidade-empresas_ie.html');