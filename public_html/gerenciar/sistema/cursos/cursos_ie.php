...
if($_POST){
	if($ac=='i' && in_array($Mod_ID."a", $_SESSION['_user']['Permissao_Modulos'])){
        $Cursos->Grava($_POST, $_FILES['midia']);
	}elseif($ac=='e' && in_array($Mod_ID."e", $_SESSION['_user']['Permissao_Modulos'])){
        $Cursos->Edita($_POST, $_FILES['midia'], $id);
	}

	header("Location:".urlBase."$Mod[0]/$Mod[1]/cursos.php");
}else{
	//TOKEN
...
$smarty->display('sistema/cursos/cursos_ie.html');