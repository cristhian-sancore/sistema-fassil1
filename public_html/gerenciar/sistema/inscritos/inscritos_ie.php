...
if($_POST){
	if($ac=='i' && in_array($Mod_ID."a", $_SESSION['_user']['Permissao_Modulos'])){
		$Inscritos->Grava($_POST);
	}elseif($ac=='e' && in_array($Mod_ID."e", $_SESSION['_user']['Permissao_Modulos'])){
		$Inscritos->Edita($_POST, $id);
	}
	header("Location:".urlBase."$Mod[0]/$Mod[1]/inscritos.php");
}else{
	//TOKEN
...
$smarty->assign('status', $Inscritos->Status_Inscricao());
$smarty->assign('cidades', $Inscritos->Cidades());
$smarty->assign('dados', $Inscritos->Dados($id));
$smarty->display('sistema/inscritos/inscritos_ie.html');