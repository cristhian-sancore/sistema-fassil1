...
if($_POST){
	if($ac=='i' && in_array($Mod_ID."a", $_SESSION['_user']['Permissao_Modulos'])){
		$SaidasExpedientes->Grava($_POST);
	}elseif($ac=='e' && in_array($Mod_ID."e", $_SESSION['_user']['Permissao_Modulos'])){
		$SaidasExpedientes->Edita($_POST, $id);
	}

	header("Location:".urlBase."$Mod[0]/$Mod[1]/saidasExpedientes.php");
}else{
	//TOKEN
...
$smarty->assign('usuarios', $SaidasExpedientes->Usuarios($_SESSION['_user']['Empresa_ID']));
$smarty->assign('cidades', $SaidasExpedientes->Cidades($_SESSION['_user']['Empresa_ID']));
$smarty->assign('dados', $SaidasExpedientes->Dados($id));
$smarty->display('sistema/atendimentos/saidasExpedientes_ie.html');